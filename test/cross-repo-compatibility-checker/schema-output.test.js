...
async function generatePacket() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "ai-tools-cross-repo-schema-"));
  await runCompatibilityCheck({
    aiToolsDir: path.join(fixtureRoot, "ai-tools"),
    aiWorkspaceKitDir: path.join(fixtureRoot, "ai-workspace-kit"),
    argv: ["--ai-tools", "fixture-ai-tools", "--ai-workspace-kit", "fixture-kit", "--out", "packet"],
...
test("generated checker packet validates against review packet schemas", async () => {
  ...
  try {
    ...
    assert.equal(packet.summary.tool.tool_name, CROSS_REPO_COMPATIBILITY_CHECKER_TOOL_NAME);
...
test("generated checker JSON artifacts are deterministic with a fixed clock", async () => {
